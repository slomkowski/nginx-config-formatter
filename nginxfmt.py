...
class Formatter:
    ...
    def count_multi_semicolon(self, single_line):
    ...
    def multi_semicolon(self, single_line):
    ...
    def apply_variable_template_tags(self, line: str) -> str:
        ...
        return re.sub(r'\${\s*(\w+)\s*}',
                      self.TEMPLATE_VARIABLE_OPENING_TAG + r"\1" + self.TEMPLATE_VARIABLE_CLOSING_TAG,
                      line,
                      flags=re.UNICODE)
    ...
    def strip_variable_template_tags(self, line: str) -> str:
        ...
        return re.sub(self.TEMPLATE_VARIABLE_OPENING_TAG + r'\s*(\w+)\s*' + self.TEMPLATE_VARIABLE_CLOSING_TAG,
                      r'${\1}',
                      line,
                      flags=re.UNICODE)
    
    
    def apply_bracket_template_tags(self, content: str) -> str:
        ...
        for c in content:
            ...
            if in_quotes:
                if c == "{":
                    result += self.TEMPLATE_BRACKET_OPENING_TAG
                elif c == "}":
                    result += self.TEMPLATE_BRACKET_CLOSING_TAG
                else:
                    result += c
            else:
                result += c
    ...
    def reverse_in_quotes_status(self, status: bool) -> bool:
        if status:
            return False
        return True
    ...
    def strip_bracket_template_tags(self, content: str) -> str:
        """ Replaces tags back with { and } respectively."""
        content = content.replace(self.TEMPLATE_BRACKET_OPENING_TAG, "{", -1)
        content = content.replace(self.TEMPLATE_BRACKET_CLOSING_TAG, "}", -1)
        return content
                    ...
                    if q == 1 and c > 1:
                        ...
                        if line.startswith("rewrite"):
                            ...
                            cleaned_lines.extend(
                                [self.strip_variable_template_tags(l).strip() for l in re.split(r"([{}])", line) if l != ""])
        return cleaned_lines
    
    
    def join_opening_bracket(self, lines):
    ...
    def perform_indentation(self, lines):
        ...
        for line in lines:
            ...
            if line != "":
                indented_lines.append(current_indent * self.INDENTATION + line)
            else:
    ...
    def format_config_contents(self, contents):
        """Accepts the string containing nginx configuration and returns formatted one. Adds newline at the end."""
        contents = self.apply_bracket_template_tags(contents)
        lines = contents.splitlines()
        lines = self.clean_lines(lines)
        lines = self.join_opening_bracket(lines)
        lines = self.perform_indentation(lines)
    ...
    def format_config_file(self, file_path, original_backup_file_path=None):
        """
        ...
        with codecs.open(file_path, 'w', encoding=chosen_encoding) as wfp:
            wfp.write(self.format_config_contents(original_file_content))
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("-i", "--indent", action="store", default=4, type=int, help="specify number of spaces for indentation")
    arg_parser.add_argument("config_files", nargs='+', help="configuration files to format")

    args = arg_parser.parse_args()
    formatter = Formatter(args.indent, args.verbose)

    for config_file_path in args.config_files:
        backup_file_path = config_file_path + '~' if args.backup_original else None
        formatter.format_config_file(config_file_path, backup_file_path)
